...
<script
	...
	<script type="text/javascript">
		...
		function loadData() {
			$
					.ajax({
						url: 'loadcontentbyauthorid/<c:out value="${userid}"></c:out>',
						dataType: 'json',
						type: 'get',
						cache: false,
							...
							$
									.each(
											...
											function (index, value) {
												...
												event_data += '<tr>';
												event_data += '<td>' + id + '</td>';
												event_data += '<td>' + value.title
														+ '</td>';
												event_data += '<td>' + value.brief
														+ '</td>';
		...
		$(document).ready(function doSearch() {
			$('#searchbox').on("input", function () {
				...
				if (s==""){
					...
					$.ajax({
						url: "searchcontentbyauthorid/"+s+"/"+<c:out value="${userid}"></c:out>,
						dataType: 'json',
						type: 'get',
						cache: false,
						success: function (data) {
									...
									.each(
											...
											function (index, value) {
												...
												event_data += '<td>'
														...
														+ '</td>';
												event_data += '<td> <button type="button" class="btn btn-success" onclick="location.href=\'update?id='+value.contentID +'\'" '       +'>Edit</button></td>';
												event_data += '<td> <button type="button" class="btn btn-success" onclick="deleteBook(' + value.contentID + ');" '      +'>Delete</button></td>';
