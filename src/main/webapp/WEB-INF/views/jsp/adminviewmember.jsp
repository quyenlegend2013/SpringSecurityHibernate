                        ...
                        $
                            .each(
                                ...
                                function (index, value) {
                                    ...
                                    event_data += '<tr>';
                                    event_data += '<td>' + id + '</td>';
                                    event_data += '<td>' + formatDate(new Date(value.createDate));+' '+value.updateTime+ '</td>';
                                    event_data += '<td>' + value.email + '</td>';
                                    event_data += '<td>' + value.firstname + '</td>';
                                    event_data += '<td>' + value.lastname + '</td>';
                                    event_data += '<td>' + value.phone + '</td>';
                                    event_data += (value.roleID.roleID != 1) ? '<td> <button type="button" class="btn btn-success" onclick="deleteMember(' + value.memberID + ');" '      +'>Delete</button></td>' : '<td> <button type="button" class="btn btn-success" disabled onclick="deleteMember(' + value.memberID + ');" '      +'>Delete</button></td>';
                                    event_data += '</tr>';
                                    event_data += '</tbody>';
                                    id++;
                                });
            ...
            <div class="col-9">
                ...
                <div class="row p-4">
                    <div class="col border border-bottom-0">
                        <div
                                class="row bg-light p-2 border border-top-0 border-left-0 ">View
                            Content List
                        </div>
                    </div>
                    <table class="table table-hover table-bordered " id="list_table_json">
                        <thead>
                        <tr>
                            <th>#</th>
                            <th>Created Date</th>
